...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const entry = await getDailyHoroscopeBySlugAndDate("cung-hoang-dao", slug, date);

  if (!entry) {
    return {};
  }

  const canonical = `${getSiteUrl()}/tu-vi-cung-hoang-dao/${entry.slug}/${entry.publishDate}`;
...
export default async function TuViCungHoangDaoDatedDetailPage({ params }: PageProps) {
  const { slug, date } = await params;
  const [entry, sidebarTopics] = await Promise.all([
    getDailyHoroscopeBySlugAndDate("cung-hoang-dao", slug, date),
    getSidebarTopics()
  ]);
          ...
          <div className="min-w-0 space-y-4">
            {date !== today ? (
              <section className="rounded-[2rem] border border-sky-200 bg-sky-50/80 px-5 py-4 text-sm leading-6 text-sky-900 shadow-card">
                Đây là bản tử vi theo ngày <strong>{date}</strong>. Để xem nội dung mới nhất, hãy quay lại danh sách 12 cung hoàng đạo.
              </section>
            ) : null}

            <DailyHoroscopeReader entry={entry} theme="zodiac" />
          </div>
        </div>
      </Container>
    </main>
