...
export default async function TuViCungHoangDaoPage() {
  const [item, sidebarTopics, entries] = await Promise.all([
    getItemBySlug("tu-vi-cung-hoang-dao"),
    getSidebarTopics(),
          ...
          <div className="min-w-0 space-y-6">
            <section className="rounded-[2rem] border border-slate-200 bg-[radial-gradient(circle_at_top_left,_#fff5ea,_#ffffff_60%)] p-6 shadow-card md:p-8">
              ...
              <p className="mt-4 max-w-4xl text-base leading-7 text-slate-700 md:text-lg">
                Xem nhanh tử vi hôm nay của 12 cung hoàng đạo qua từng trang đọc riêng, trình bày
                ngắn gọn, rõ ràng và thuận tiện để tra cứu hằng ngày.
              </p>
            </section>

            <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-card md:p-8">
              <div className="border-b border-slate-200 pb-5">
                <h2 className="text-2xl font-bold text-slate-900">Tử vi hôm nay theo 12 cung hoàng đạo</h2>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Chọn một cung để mở trang đọc tử vi của ngày hôm nay.
                </p>
              </div>
              <HoroscopeOverviewGrid entries={entries} accentClassName="text-blue-700" />
            </section>
          </div>
        </div>
      </Container>
